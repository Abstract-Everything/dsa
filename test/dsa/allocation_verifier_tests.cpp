...
using Single_Field_Allocator    = dsa::Memory_Monitor<Empty_Value, Allocation_Verifier>;
...
TEST_CASE("Monitor detects memory leak", "[allocation_verifier]") {
	...
	REQUIRE_THROWS_WITH(Allocation_Verifier::instance()->cleanup(), memory_leaked);
}
...
TEST_CASE("An allocate followed by a valid deallocate raises no error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("Allocations can only be deallocated once", "[allocation_verifier]") {
	...
	REQUIRE_THROWS_WITH(Allocation_Verifier::instance()->cleanup(), deallocating_unallocated_memory);
}

TEST_CASE("Multiple allocations each require a deallocation", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("Deallocating constructed elements raises an error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
	...
	REQUIRE_THROWS_WITH(Allocation_Verifier::instance()->cleanup(), object_leaked);
}

TEST_CASE("Constructed elements left undestroyed raise an error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("Objects initialised through a copy also require destruction", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("Copy construct from uninitialised memory raises an error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("Move construct from uninitialised memory raises an error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("Copy assign from uninitialised memory raises an error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("Move assign from uninitialised memory raises an error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("Assign to uninitialized memory raises an error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
	...
	*memory = value;
	...
	REQUIRE_THROWS_WITH(Allocation_Verifier::instance()->cleanup(), assign_uninitialized_memory);
}
...
TEST_CASE("Values can be used to move construct only once", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
	...
	REQUIRE_THROWS(Allocation_Verifier::instance()->cleanup(), assign_from_uninitialized_memory);
}
...
TEST_CASE("Calling destroy on unconstructed memory raises an error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("A construct followed by a destroy raises no error", "[allocation_verifier]") {
	Handler_Scope          scope;
	Single_Field_Allocator verifier;
...
TEST_CASE("Objects allocated on the stack do not raise errors", "[allocation_verifier]") {
	Handler_Scope scope;
	{ Single_Field_Allocator::Value value; }
	REQUIRE_NOTHROW(Allocation_Verifier::instance()->cleanup());
}
...
struct Multiple_Field_Struct
{
	...
	friend auto operator<<(std::ostream &stream, Multiple_Field_Struct const &data)
	    -> std::ostream & {
		return stream << '{' << data.field_a << ',' << data.field_b << '}';
	}
};

using Multiple_Field_Allocator = dsa::Memory_Monitor<Multiple_Field_Struct, Allocation_Verifier>;

using Multiple_Field_Alloc_Traits = dsa::Allocator_Traits<Multiple_Field_Allocator>;

TEST_CASE("Complex structures can have overlapping construct calls", "[allocation_verifier]") {
	Handler_Scope            scope;
